...
public:
    ...
    std::vector<uint32_t> GetBlockInt(const uint32_t &vertex);

    // return true if fully encode
    bool GetFlag(const uint32_t &vertex) {
        return encode_bitset_[vertex].BlockGet(0, v_bits_size_);
    };

    // flag == true means fully encode
    void SetFlag(const uint32_t &vertex, bool flag) {
