...
public:
     ...
     * */
    virtual bool Get(const uint32_t &vertex1, const uint32_t &vertex2) = 0;

    // encode edge<vertex1,vertex2>
    virtual void Set(const uint32_t &vertex1, const uint32_t &vertex2);

    /**
     *  encode one vertex by it's neighbors
     *
     * */
    virtual void EncodeVertex(const uint32_t &vertex_id, const std::vector<uint32_t> &neighbors);
...
private:
    ...
    uint32_t vertex_id_upper_ = VERTEX_SIZE + 1;
    // equals vertex_id_upper_ -1
    uint32_t vertex_size_ = VERTEX_SIZE;
};
