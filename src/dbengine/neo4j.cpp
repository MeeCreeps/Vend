...
bool Neo4j::Get(uint32_t vertex, std::vector<uint32_t> *neighbors) {
    assert(connection_ != NULL);
    char buf[1024];
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "MATCH (a:person)-[:knows]-(b) WHERE a.id ='%d' RETURN toInteger(b.id)", vertex);
...
bool Neo4j::Put(uint32_t vertex1, uint32_t vertex2) {
    assert(connection_ != NULL);
    char buf[128];
    memset(buf, 0, sizeof(buf));
    sprintf(buf,
...
bool Neo4j::QueryEdge(uint32_t vertex1, uint32_t vertex2) {
    ...
    memset(buf, 0, sizeof(buf));
    sprintf(buf,
            "MATCH (a:person)-[r]-(b:person)\nWHERE a.id = '%d' AND b.id = '%d' return a,r,n\n",
            vertex1, vertex2);
    neo4j_result_stream_t *results = neo4j_run(connection_, buf, neo4j_null);
    neo4j_result_t *result = neo4j_fetch_next(results);
    if(result==NULL){
